...
public class HexDump
{
	...
	public synchronized static void dump(String s)
	{
	...
	public synchronized static void dump(byte[] b)
	{
		for (int i = 0; i < b.length; i++)
		{
			...
			if ((i + 1) % bytes_per_row == 0)
			{
				...
				Log.println(BAC.toString(b).substring(i + 1 - bytes_per_row,
						i + 1));
			} else if (i + 1 == b.length)
			{
				...
				Log.print(bytes_string_sep);
				Log.println(BAC.toString(b).substring(
						i + 1 - ((i + 1) % bytes_per_row), i + 1));
			}
		}
	}
}
